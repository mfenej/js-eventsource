...
function EventSource (url, eventSourceInitDict) {
  ...
  function connect () {
    ...
    req = api.request(urlAndOptions.options, callback)
  ...
  this.setReadyState = function(newState) {
    if (newState === EventSource.CONNECTING || newState === EventSource.OPEN || newState === EventSource.CLOSED) {
        readyState = newState;

        // Emit appropriate events based on newState
        switch (newState) {
            case EventSource.CONNECTING:
                _emit(new Event('connecting'));
                break;
        ...
        throw new Error('Invalid readyState value');
    }
};
